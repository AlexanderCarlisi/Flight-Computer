...
#define RF_CHNL           110
#define RF_TX             {'P', 'A', 'T', '0', '2', '\0'}
#define RF_RX             {'P', 'A', 'T', '0', '1', '\0'}
#define RF_CE             9
#define RF_CSN            8
...
// void transmit(RF24& radio, byte payload[]) {
...
void recieve(RF24& radio) {
  uint8_t pipe;
  if (radio.available(&pipe)) {
    // uint8_t bytes = radio.getPayloadSize();
    radio.read(&payload, PAYLOAD_SIZE);
    Serial.print(F("Received "));
    Serial.print(PAYLOAD_SIZE);  // print the size of the payload
    Serial.print(F(" bytes on pipe "));
    Serial.print(pipe);  // print the pipe number
    Serial.print(F(": \n"));
    for (int i = 0; i < PAYLOAD_SIZE; i++) {
      Serial.println((int) payload[i]);  // print the payload's value
    }
    Serial.println("\n");
  }
}
...
void loop() {
  // transmit(rf24_radio, "Hello World");
  recieve(rf24_radio);
  delay(1000);
}
